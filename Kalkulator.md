...
W danych wejściowych występował niepoprawny wariant nazwy. Poprawna angielska nazwa wpisu `race_15` to `Ethereal Caste` i taka forma została użyta w tłumaczeniach oraz w tabeli danych do przeniesienia.

## Zakres zmian w CharacterCreation.html

W pliku `CharacterCreation.html` rozszerzono wyłącznie struktury używane przez pomocniczą tabelę maksymalnych wartości atrybutów:
...
## Zakres zmian w XPCalculator.html

W pliku `XPCalculator.html` rozszerzono wyłącznie struktury używane przez pomocniczą tabelę maksymalnych wartości atrybutów:
...
2. Znajdź tablicę z wierszami ras/gatunków. W tym repozytorium są to:
   - `maxAttributeRows` w `CharacterCreation.html`,
   - `attributeMaximumRows` w `XPCalculator.html`.
...
11. Po przeniesieniu sprawdź widok tabeli w wersji polskiej i angielskiej oraz wyszukaj niepoprawny wariant nazwy.
...
Główne ryzyko polega na błędnym utożsamieniu wartości `Szybkość = 14` dla Vespidów z limitem edytowalnego pola w kalkulatorze. W tej zmianie ta wartość jest informacyjna i dotyczy tylko tabeli pomocniczej. Drugim ryzykiem jest przepisanie niepoprawnego wariantu nazwy zamiast poprawnej formy `Ethereal Caste`.
...
W pliku `CharacterCreation.html` przy wartości `14` w pomocniczej tabeli maksymalnych wartości atrybutów dodano prezentacyjną gwiazdkę wyłącznie dla komórki:
...
`XPCalculator.html` celowo nie otrzymuje gwiazdki ani przypisu. Osobny kalkulator PD nadal pokazuje Vespidów / Vespid z wartością Szybkości `14` bez oznaczenia `*`, ponieważ przypis dotyczy tylko generatora postaci i ręcznej interpretacji pola Szybkość przy tworzeniu Vespidów.
...
- Gwiazdka i przypis występują tylko w `CharacterCreation.html`.
- W `XPCalculator.html` wartość `14` pozostaje bez gwiazdki i bez przypisu.
...
- [ ] W `CharacterCreation.html` przy Vespidach/Vespidach w kolumnie Szybkość/Speed widoczna jest wartość `14*`.
...
- [ ] W `XPCalculator.html` nie ma gwiazdki przy wartości `14`.
- [ ] W `XPCalculator.html` nie ma przypisu dla Vespidów.
- [ ] Nie zmieniono `attributeCosts`.
- [ ] Nie zmieniono `skillCosts`.
- [ ] Nie zmieniono limitów inputów.
- [ ] Nie zmieniono mechaniki przeliczania PD.
- [ ] Nie występuje niepoprawny wariant nazwy.
